...
for i in range(n):
    ...
    # -------------2 система------------------
    sig4[i] = 2.5*cos(f2w(fc[3]-0.2)*time[i])+2.5*cos(f2w(fc[3])*time[i])+3.5*cos(f2w(fc[3]+0.2)*time[i])
    sig5[i] = 3.0*cos(f2w(fc[4]-0.2)*time[i])+3.5*cos(f2w(fc[4])*time[i])+4.0*cos(f2w(fc[4]+0.2)*time[i])
    sig6[i] = 3.5*cos(f2w(fc[5]-0.2)*time[i])+4.0*cos(f2w(fc[5])*time[i])+4.5*cos(f2w(fc[5]+0.2)*time[i])
    # ----------------------------------------
#--------------------------------------------------------------------------------------------------------
...
sp_mch_1_3 = np.hypot(sp_mch_1_2.real, sp_mch_1_3.imag)/n*2
#----------------------------------------

#-------------2 система------------------
sp_mch_2_1 = np.hypot(sp_mch_2_1.real, sp_mch_1_1.imag)/n*2
sp_mch_2_2 = np.hypot(sp_mch_2_2.real, sp_mch_2_2.imag)/n*2
sp_mch_3_3 = np.hypot(sp_mch_2_3.real, sp_mch_2_3.imag)/n*2
#----------------------------------------
#--------------------------------------------------------------------------------------------------------
...
for i in range(n):
    mch1[i] = (1.0 + m * sp_tgrp123[i])*cos(f2w(fn[0])*time[i])
    mch2[i] = (1.0 + m * sp_tgrp456[i])*cos(f2w(fn[1])*time[i])
#--------------------------------------------------------------------------------------------------------
...
sp_mch_2_1 = np.fft.fft(mch_1_1)
sp_mch_2_2 = np.fft.fft(mch_1_2)
sp_mch_2_3 = np.fft.fft(mch_1_3)
